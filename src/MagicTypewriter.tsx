...
`;
export const MagicTypewriter: React.FC<ContainerProps> = ({
  message,
  onComplete,
  splitLines,
  splitString,
  ...
  useEffect(() => {
    ...
    const totalChars = lineCharLength.reduce(
    ...
    splitText.map((line: string[][], lineIndex: number) => {
      ...
      const prevLineChars = splitText
        .map((thisLine: string[][], lineOrder: number) => {
          let currentLineLength = 0;
      ...
      return (timeouts["lines"] = setTimeout(() => {
        ...
        thisLine.map((currentWord: string[], wordIndex: number) => {
          ...
          const prevWordChars = line
            .map((thisWord: string[], wordOrder: number) => {
              let currentLineLength = 0;
            ...
            allLines.map((line: string[][], lineIndex: number) => {
              return (
                <div
                  ...
                  style={{ display: "block" }}
                >
                  {allLines[lineIndex].map(
                    (word: string[], wordIndex: number) => {
                      return (
                        <div
                          aria-hidden="true"
                          className="promptWord"
                          key={wordIndex}
                          style={{
                            letterSpacing: "0",
                        ...
                        >
                          {allLines[lineIndex][wordIndex].map(
